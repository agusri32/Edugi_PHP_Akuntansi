...
<form name="postform" action="laporan-jurnal-umum.php" method="POST">
<p></p>
<table >
	<tr>
		<td>ID User</td>
...
if(isset($_POST['tampilkan'])){
	...
	$query=mysqli_query($link,"select * from tabel_akuntansi_transaksi where ".$q_user." jenis_transaksi='Jurnal Umum' and tanggal_transaksi between '$tanggal1' and '$tanggal2' order by tanggal_transaksi asc");
	$total=mysqli_fetch_array(mysqli_query($link,"select sum(debet) as tot_debet, sum(kredit) as tot_kredit from tabel_akuntansi_transaksi where ".$q_user." jenis_transaksi='Jurnal Umum' and tanggal_transaksi between '$tanggal1' and '$tanggal2' order by nomor_perkiraan asc"));

}else{
	unset($_POST['tampilkan']);
	
	$query=mysqli_query($link,"select * from tabel_akuntansi_transaksi where jenis_transaksi='Jurnal Umum' order by tanggal_transaksi asc");
	$total=mysqli_fetch_array(mysqli_query($link,"select sum(debet) as tot_debet, sum(kredit) as tot_kredit from tabel_akuntansi_transaksi where jenis_transaksi='Jurnal Umum' order by nomor_perkiraan asc"));
}
?>
...
<thead>
	<tr>
		...
		<th width="100" class=hr>Tipe</th>
		<th width="75" class=hr>No.Prk</th>
		<th width="182" class=hr>Nama Perkiraan</th>
		<th width="141" class=hr>Keterangan</th>
		<th width="114" class=hr><div align="right">Debet</div></th>
		<th width="93" class=hr><div align="right">Kredit</div></th>
...
while($row=mysqli_fetch_array($query)){
	...
	<tr>
		...
		<td><div align="center"><?php echo $id_user;?></div>
		</td>
	</tr>
	<?php
}	
