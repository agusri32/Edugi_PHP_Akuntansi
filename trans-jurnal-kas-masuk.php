...
if(isset($_POST['next'])){
	...
	$input_transaksi=mysqli_query($link,"INSERT INTO tabel_akuntansi_transaksi(kode_jurnal, nomor_perkiraan, tanggal_transaksi, jenis_transaksi, keterangan_transaksi, kredit, id_user, bulan_transaksi) 
		values('$kode_jurnal','$nomor_perkiraan','$tanggal_transaksi','Kas Masuk','$keterangan','$jumlah_transaksi','$id_user','$bulan')");
...
if(isset($_POST['selesai'])){
	...
	$query_transaksi=mysqli_query($link,"insert into tabel_akuntansi_transaksi(kode_jurnal,nomor_perkiraan,tanggal_transaksi, bulan_transaksi, jenis_transaksi, keterangan_transaksi,debet,id_user)
		values('$kode_jurnal','$nomor_kas_induk','$tanggal_selesai','$bulan','Kas Masuk','$keterangan_jurnal','$total_kas','$id_user')");
	
	if($query_transaksi){
		$query_jurnal=mysqli_query($link,"insert into tabel_akuntansi_jurnal_masuk(nomor_jurnal, kode_jurnal, kode_invoice,tanggal_selesai) values('$nomor_jurnal','$kode_jurnal', '$keterangan_invoice','$tanggal_selesai')");
		...
		?><script language="javascript">document.location.href="trans-jurnal-kas-masuk.php"</script><?php
...
$jurnal_masuk=mysqli_fetch_array(mysqli_query($link,"SELECT max(nomor_jurnal) as nomor_jurnal FROM tabel_akuntansi_jurnal_masuk ORDER BY tanggal_selesai DESC"));
$nomor_jurnal=$jurnal_masuk['nomor_jurnal']+1;
$kode_jurnal="JKM/".$nomor_jurnal;
?>

<form name="postform" action="trans-jurnal-kas-masuk.php" method="POST">
<h2>Transaksi >> Transaksi GL &gt;&gt; Jurnal Kas Masuk</h2> 
	...
	<td align="left" colspan="2">
		<table id='rounded-corner'>
			...
			<tfoot>				
				<tr>
					<td class='rounded-foot-left'>
						<div id="suggest">
						   <input type="text" autocomplete="off" onKeyUp="suggest(this.value);" name="nomor_perkiraan"  onBlur="fill2();" id="kode" size="5"/> 
						   <div class="suggestionsBox" id="suggestions" style="display: none;">
						   <div class="suggestionList" id="suggestionsList"> &nbsp; </div>
						   </div>
						</div>								
			...
			while($row=mysqli_fetch_array($query_transaksi)){
				...
				?>
				<tr>
				<td><?php echo $kode_prk; ?></td><td><?php echo ucwords($nama_prk); ?></td><td><?php echo ucwords($q_keterangan); ?></td><td align="right"><?php echo number_format($kredit,2,'.',','); ?></td>
				<td><a href="trans-jurnal-kas-masuk.php?mode=cancel&id=<?php echo $id_transaksi;?>" onClick="return confirm('Apakah Anda yakin ?')">Cancel</a></td>
				</tr>
				<?php
			}
			?>
...
<a href='trans-jurnal-kas-masuk.php?mode=reset&id=<?php echo $kode_jurnal; ?>' onClick="return confirm('Apakah Anda yakin ?')"><font color='blues'>Reset All Jurnal</font></a>
</fieldset>	
</form>	

<form method="POST" action="trans-jurnal-kas-masuk.php" method="post">
<?php
$query_sum=mysqli_fetch_array(mysqli_query($link,"select sum(kredit) as tot_kredit from tabel_akuntansi_transaksi where kode_jurnal='$kode_jurnal' and id_user='$id_user'"));
$tot_kredit=$query_sum['tot_kredit'];
?>
...
<input type="hidden" name="total_kas" value="<?php echo $tot_kredit;?>">
...
<tr>
	...
	<td width="32%"><input type="text" size="30" disabled="disabled" value="<?php echo number_format($tot_kredit,2,'.',','); ?>"></td>
</tr>
<tr>
