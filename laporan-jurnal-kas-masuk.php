...
<div>
<h2>Laporan >> Laporan Jurnal Kas Masuk </h2> 
<fieldset style="border:solid 2px #ccc;">
<legend><strong>PERIODE : </strong></legend>

<form name="postform" action="laporan-jurnal-kas-masuk.php" method="POST">
<p></p>
<table >
	<tr>
		<td>ID User</td>
...
if(isset($_POST['tampilkan'])){
	...
	$query=mysqli_query($link,"select * from tabel_akuntansi_transaksi where ".$q_user." jenis_transaksi='Kas Masuk' and tanggal_transaksi between '$tanggal1' and '$tanggal2' order by tanggal_transaksi asc");
	$total=mysqli_fetch_array(mysqli_query($link,"select sum(debet) as tot_debet, sum(kredit) as tot_kredit from tabel_akuntansi_transaksi where ".$q_user." jenis_transaksi='Kas Masuk' and tanggal_transaksi between '$tanggal1' and '$tanggal2' order by nomor_perkiraan asc"));

}else{
	unset($_POST['tampilkan']);
	
	$query=mysqli_query($link,"select * from tabel_akuntansi_transaksi where jenis_transaksi='Kas Masuk' order by tanggal_transaksi asc");
	$total=mysqli_fetch_array(mysqli_query($link,"select sum(debet) as tot_debet, sum(kredit) as tot_kredit from tabel_akuntansi_transaksi where jenis_transaksi='Kas Masuk' order by nomor_perkiraan asc"));
}
?>
