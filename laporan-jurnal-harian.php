...
<div>
<h2>Laporan >> Laporan Jurnal Harian</h2> 
<fieldset style="border:solid 2px #ccc;">
<legend><strong>PERIODE : </strong></legend>

<form name="postform" action="laporan-jurnal-harian.php" method="POST">
<p></p>
<table >
	<tr>
		<td>ID User</td>
...
if(isset($_POST['tampilkan'])){
	...
	$query=mysqli_query($link,"select * from tabel_akuntansi_transaksi where ".$q_user." tanggal_transaksi between '$tanggal1' and '$tanggal2' order by tanggal_transaksi asc");
	$total=mysqli_fetch_array(mysqli_query($link,"select sum(debet) as tot_debet, sum(kredit) as tot_kredit from tabel_akuntansi_transaksi where ".$q_user." tanggal_transaksi between '$tanggal1' and '$tanggal2' order by nomor_perkiraan asc"));
}else{
	unset($_POST['tampilkan']);
	
	$query=mysqli_query($link,"select * from tabel_akuntansi_transaksi order by tanggal_transaksi asc");
	$total=mysqli_fetch_array(mysqli_query($link,"select sum(debet) as tot_debet, sum(kredit) as tot_kredit from tabel_akuntansi_transaksi order by nomor_perkiraan asc"));
}
?>
...
<thead>
	<tr>
		...
		<th class=hr>No.Jurnal</th>
		<th class=hr>Tipe</th>
		<th class=hr>No.Prk</th>
		<th class=hr>Nama Perkiraan</th>
		<th class=hr>Keterangan</th>
		<th class=hr><div align="right">Debet</div></th>
		<th class=hr><div align="right">Kredit</div></th>
