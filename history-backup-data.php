<?php include "koneksi.php"; ?>
	...
	<div>
	<h2>Maintenance >> History Backup Database </h2> 
	<fieldset style="border:solid 2px #ccc;">
		...
		$query=mysqli_query($link,"select * from tabel_akuntansi_history_backup order by tanggal_backup desc");
		while($row=mysqli_fetch_array($query)){
			...
			<td>
			<div align="center">
			<a  href="./content/backup/download.php?f=<?php echo $row['nama_file'];?>" title="Download"><img  border="0" src="./images/save.jpeg"/></a> 
			<a href="history-backup-data.php?nama_file=<?php echo $row['nama_file'] ?>&mode=delete" onclick="return confirm('Apakah Anda yakin akan menghapus data ini?')" title="Delete"><img border="0" src="./images/delete.png"/></a>
			</span>
			</div>
			</td>
			</tr>
		...
		<tr>
			...
			if($mode=='delete'){
				$delete=mysqli_query($link,"delete from tabel_akuntansi_history_backup where nama_file='$nama_file'");
				unlink("./content/backup/data/".$nama_file."");
				
				if($delete){
					?><script language="javascript">document.location.href="history-backup-data.php?status=Data berhasil di hapus"</script><?php
				}else{
					echo mysql_error();
				}
			}
		...
		<tr>
			...
			<b>TOTAL FILE BACKUP :
			<?php 
			$total=mysqli_query($link,"select * from tabel_akuntansi_history_backup"); 
